...
const ProjectsPage = () => {
    ...
    // Action states (Restricted for users, but kept for type compatibility)
    const [deletingProject, setDeletingProject] = useState<Project | null>(null);
    const [archivingProject, setArchivingProject] = useState<Project | null>(null);
    const [actionLoading, setActionLoading] = useState(false);

    const limit = 8; // Multiple of 4/2 for better grids
    ...
    return (
        <UserLayout>
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6 relative z-10">
                        <div className="space-y-2">
                            ...
                            <h1 className="text-3xl sm:text-4xl lg:text-5xl font-black text-text-primary tracking-tight leading-none">
                                My <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary via-accent to-[#38bdf8]">Projects</span>
                            </h1>
                            <p className="text-sm sm:text-base text-text-secondary max-w-xl font-medium">
                                View and manage work portals you are currently active in.
                            </p>
                        </div>
                ...
                <div className="bg-surface/30 backdrop-blur-md border border-border-theme rounded-3xl p-5 sm:p-6 shadow-sm space-y-4">
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                        ...
                        <div className="sm:col-span-2 relative group">
                            <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-4.5 h-4.5 text-text-secondary group-focus-within:text-primary transition-colors" />
                            <input
                                type="text"
                                placeholder="Search project title or description..."
                                value={searchQuery}
                                onChange={handleSearchChange}
                                className="w-full bg-background/50 border border-border-theme rounded-2xl pl-12 pr-4 py-3.5 text-text-primary placeholder:text-text-secondary focus:outline-none focus:ring-4 focus:ring-primary/5 focus:border-primary transition-all text-xs font-semibold"
                            />
                        </div>
                        ...
                        <div className="relative">
                            ...
                            <select
                                ...
                                className="w-full bg-background/50 border border-border-theme rounded-2xl pl-10 pr-8 py-3.5 text-text-primary focus:outline-none focus:ring-4 focus:ring-primary/5 focus:border-primary transition-all appearance-none cursor-pointer font-bold text-xs"
                            >
                ...
                {loading ? (
                    ...
                    <div className="bg-surface/30 backdrop-blur-md border border-border-theme rounded-3xl p-12 text-center shadow-inner">
                        ...
                        <h3 className="text-base font-bold text-text-primary mb-1">
                            {searchQuery ? "No matching projects" : "No assignments found"}
                        </h3>
                        <p className="text-xs text-text-secondary max-w-xs mx-auto mb-6">
                            {searchQuery 
                                ...
                                : "You have not been assigned to any workspaces. Contact your coordinator to get access."
                            }
                        </p>
                        {searchQuery && (
                            <button
            ...
            {/* Confirmation Modals - Restricted for regular users but defined for layout consistency */}
            <ConfirmModal
